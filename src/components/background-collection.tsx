...
export default function BackgroundCollection(props: Props) {
    ...
    function handleRandomPosition() {
        ...
        range(0, props.count).forEach((index) => {
            ...
            let retries = 0;
            while (checkOverlap(top, left, newImagePositions, props.minDistance) && retries < MAX_RETRIES) {
                top = generateTop();
                left = generateLeft();
    ...
    function checkOverlap(
        ...
        for (const position of Object.values(newImagePositions)) {
            const xOverlap = position.left + props.imageWidth + minDistance > left && position.left - minDistance < left + props.imageWidth;
            const yOverlap = position.top + props.imageHeight + minDistance > top && position.top - minDistance < top + props.imageHeight;
